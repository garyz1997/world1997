...
k = open('../data/forms.txt','r')
c = k.readlines()
k.close()
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~search results (derek)~~~~~~~~~~~~~~~~~~~~~~~~
searchResults=''
if 'search' in D:
    searchResults += '<hr><b>Users</b><br>'
    for user in users:
        if D['search'] in user[0]:
            searchResults += '<a href="profile.py?username=' + user[0] + '">' + user[0] + '</a><br>'
    searchResults += '<hr><b>Reddit</b><br>'
    for lines in c:
        if D['search'] in lines.split(',')[0]:
            searchResults += lines.split(',')[1].strip('\n') + '&nbsp;&nbsp;&nbsp;<i>' + lines.split(',')[0] +'</i>\'s post<br>'
else:
    searchResults = '<p>Error.  Try searching again!</p>'
