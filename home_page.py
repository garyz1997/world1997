...
password2= users[userNumber][1]

d=md5.new()                             
d.update(password2)
hashedpass2=str(d.hexdigest())
#BEGIN EDIT BY DEREK
if D['user'] in data and hashedpass==password2:
    j=open('../data/loggedin.txt', 'r')
    number=j.read()
    j.close()
    lines=[]
    ...
    if D['user'] in number and cgi.escape(os.environ["REMOTE_ADDR"]) == lines[userNum][2] and date == lines[userNum][3]:
        s=open('../data/loggedin.txt', 'w')
        s.write(number)
        s.close()
    ...
    #NEW USER
        ...
        s.write(D['user'] + ',1,' + str(cgi.escape(os.environ["REMOTE_ADDR"])) + ',' + time.strftime("%Y%m%d") +'\n')
        s.close()
